...
def api(request, action=None):
    ...
    if action is None:
        response['endpoints'] = ['cameras/', 'commands/']

    elif action == 'commands':
        if request.META['REQUEST_METHOD'] == 'GET':
            set = CameraCommand.objects.filter(
                time_completed__isnull=True).order_by('date_added')
            response['items'] = set
        print response
        # print request

    elif action == 'updateCameras':
        ...
        for camera in camera_set:
            ...
            # only need to send bulk of data if the client hasn't seen this
            # object before
            camera.status = json.loads(camera.status)
            if not lastUpdate or (
                    camera.last_attempt is not None and
                    camera.last_attempt >= lastUpdate) or \
                    camera.date_added >= lastUpdate:
        ...
        camera_set_attempt = Camera.objects.filter(
            last_attempt__gte=timezone.now()-timedelta(seconds=30))
        ...
        # build list
        command_set = CameraCommand.objects.filter(
            time_completed__isnull=True).order_by('date_added')
        response['list'] = []
